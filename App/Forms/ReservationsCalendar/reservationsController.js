...
app.controller('MainCtrl', function ($scope, $http, $log, $location, spFormFactory) {    
    ...
    spFormFactory.initialize($scope, 'Senate Room Reservations').then(init);
    ...
    function init() {
        ...
        if($scope.formMode == 'New') {   
            $scope.f.RoomApprovalStatus.Value = "Waiting for Room Approver";   
            $scope.CurrentUserPartOfWorkflow = true;
            ...
            $scope.$watch('f.OptionalApproverUser.user.Title',function(){
                if($scope.f.OptionalApproverUser.user.Title){
                    $scope.f.RoomApprovalStatus.Value = "Waiting for Optional Approver";
                }
            });
        }
        ...
        $scope.$watch('[f.StartTime.Value, f.EndTime.Value]',function(){
            ...
            $scope.f.EventDate.Value = new Date($scope.f.DateOfUse.Value.getFullYear(), $scope.f.DateOfUse.Value.getMonth(), $scope.f.DateOfUse.Value.getDate(), 
            $scope.f.StartTime.Value.getHours(), $scope.f.StartTime.Value.getMinutes(), $scope.f.StartTime.Value.getSeconds());

            $scope.f.EndDate.Value = new Date($scope.f.DateOfUse.Value.getFullYear(), $scope.f.DateOfUse.Value.getMonth(), $scope.f.DateOfUse.Value.getDate(), 
            $scope.f.EndTime.Value.getHours(), $scope.f.EndTime.Value.getMinutes(), $scope.f.EndTime.Value.getSeconds());           
        });
